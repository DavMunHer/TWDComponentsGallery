...
export class AuthPlaygroundComponent implements OnInit {
  protected selectedTemplate: WritableSignal<
    'classicB&W' | 'neoViolet' | 'custom'
  > = signal('classicB&W');
  protected componentVisible = signal(true);

  protected customColorsForm = new FormGroup({
    primaryColor: new FormControl('#111111', { nonNullable: true }),
    secondaryColor: new FormControl('#ffffff', { nonNullable: true }),
    inputColor: new FormControl('#dddddd', { nonNullable: true }),
  });

  protected templateForm = new FormGroup({
    selectedTemplate: new FormControl<'classicB&W' | 'neoViolet' | 'custom'>(
      'classicB&W',
      { nonNullable: true }
    ),
  });
